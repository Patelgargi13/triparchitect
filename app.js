// ════════════════════════════════════════════════
//   TripArchitect — app.js  (Professional Redesign)
//   By Gargi Patel
// ════════════════════════════════════════════════

/* ══════════════════════════════════════
   DESTINATION IMAGE MAP
   Used in itinerary cards
══════════════════════════════════════ */
const DEST_IMAGES = {
...
let slideTimer = setInterval(() => goSlide(), 5000);
...
(function initSplash() {
  ...
  // Animation sequence: welcome text → plane flies → trail grows → logo reveals → tagline
  setTimeout(() => {
    // Step 1: Show "WELCOME TO" + "GARGI PATEL'S"
    if (welcome) welcome.classList.add("show");
    if (creator) creator.classList.add("show");

    setTimeout(() => {
      // Step 2: Plane flies across
      splane.classList.add("fly");

      setTimeout(() => {
        // Step 3: Trail + Logo appear
...
async function genItinerary(dest, days, travelers, budget, style, startDate, endDate, key) {
  const prompt = `You are an expert travel planner. Create a vivid, detailed ${days}-day itinerary for ${dest}.

Trip details: Travelers: ${travelers} · Budget: ${budget} · Style: ${style} · Dates: ${startDate} to ${endDate}

Format EXACTLY like this (no extra text before DAY 1):
DAY 1: [Exciting Day Theme Title]
MORNING: [Detailed morning — specific places, how to get there, local tips, timings]
AFTERNOON: [Detailed afternoon — specific landmarks, experiences, food spots]
EVENING: [Evening — restaurants, nightlife, sunset spots, local experiences]
LOCAL TIP: [One unique insider tip for this day]
---
DAY 2: [Theme]
...
async function fetchWeather(dest) {
  try {
    ...
    return { ...wData, cityName:name, countryName:country, lat:latitude, lon:longitude };
  } catch (e) { return null; }
}
...
async function fetchCountry(dest) {
  const queries = [dest.split(",")[0].trim(), dest];
...
async function fetchPhrases(dest) {
  const travelPhrases = [
    { eng:"Hello",               display:"Hello" },
    { eng:"Thank you",           display:"Thank you" },
    { eng:"Please",              display:"Please" },
    { eng:"Help",                display:"Help!" },
    { eng:"Where is",            display:"Where is...?" },
    { eng:"How much",            display:"How much?" },
    { eng:"Good morning",        display:"Good morning" },
    { eng:"Goodbye",             display:"Goodbye" },
    { eng:"Sorry",               display:"Sorry / Excuse me" },
    { eng:"I do not understand", display:"I don't understand" },
    { eng:"Cheers",              display:"Cheers!" },
    { eng:"Delicious",           display:"Delicious!" },
  ];
  const langMap = {
    france:"fr",paris:"fr",lyon:"fr",
    spain:"es",madrid:"es",barcelona:"es",mexico:"es",
    japan:"ja",tokyo:"ja",osaka:"ja",kyoto:"ja",
    germany:"de",berlin:"de",munich:"de",
    italy:"it",rome:"it",milan:"it",venice:"it",
    china:"zh",beijing:"zh",shanghai:"zh",
    brazil:"pt",portugal:"pt",lisbon:"pt",
    russia:"ru",moscow:"ru",
    korea:"ko",seoul:"ko",
    arabic:"ar",dubai:"ar",egypt:"ar",cairo:"ar",
    bali:"id",indonesia:"id",jakarta:"id",
    thailand:"th",bangkok:"th",
    greece:"el",athens:"el",santorini:"el",
    netherlands:"nl",amsterdam:"nl",
    turkey:"tr",istanbul:"tr",
    india:"hi",delhi:"hi",mumbai:"hi",
    vietnam:"vi",hanoi:"vi",
    poland:"pl",warsaw:"pl",
    czech:"cs",prague:"cs",
  };
  ...
  for (const p of travelPhrases) {
    ...
    } catch (e) { results.push({ orig:p.display, trans:p.eng }); }
  }
  return results;
}
...
Include 18-20 items: clothing, footwear, toiletries, documents, tech, health, destination-specific.`, key, 700);
...
Use correct real numbers for the country.`, key, 300);
  try { return JSON.parse(raw.replace(/```json|```/g,"").trim()); }
  catch (e) {
    return [
...
{"accommodation":50,"food":30,"transport":15,"activities":20,"misc":10,"currency":"USD","note":"A helpful money tip"}`, key, 250);
  try { return JSON.parse(raw.replace(/```json|```/g,"").trim()); }
  catch (e) { return {accommodation:60,food:35,transport:15,activities:25,misc:10,currency:"USD",note:"Prices vary by season — book in advance for better rates."}; }
}
...
/* ══════════════════════════════════════
   RENDER: ITINERARY — MAGAZINE CARD STYLE
   With destination image per day
══════════════════════════════════════ */
function renderItinerary(text, dest, days) {
  ...
  // Day-specific Unsplash queries for variety
  const dayThemeImages = [
    destImg,
  ...
  dayHeaders.forEach((header, i) => {
    ...
    const imgUrl    = dayThemeImages[i % dayThemeImages.length];
...
function renderWeather(data, dest) {
  ...
  if (!data || !data.current) {
    out.innerHTML = `<p style="text-align:center;padding:40px;color:var(--gray);">⚠️ Weather data unavailable for "${dest}". Try a major city name.</p>`;
    return;
  }
  const c = data.current;
  const wIcons = {0:"☀️",1:"🌤️",2:"⛅",3:"☁️",45:"🌫️",48:"🌫️",51:"🌦️",53:"🌦️",61:"🌧️",63:"🌧️",71:"❄️",73:"❄️",80:"🌧️",81:"🌧️",95:"⛈️",96:"⛈️"};
  ...
  out.innerHTML = `
    <div class="weather-widget">
      ...
      <div style="display:flex;align-items:flex-end;gap:16px;margin-top:8px;">
        ...
        <div>
          <div class="w-temp">${c.temperature_2m?.toFixed(1)}°C</div>
          <div class="w-desc">Current Conditions</div>
        </div>
      </div>
      <div class="w-grid">
        ...
        <div class="w-item"><div class="w-item-label">Code</div><div class="w-item-val">${icon}</div></div>
      </div>
    </div>
...
function renderCountry(data, dest) {
  ...
  if (!data) {
    out.innerHTML = `<p style="text-align:center;padding:40px;color:var(--gray);">⚠️ Country info not found for "${dest}".</p>`;
    return;
  }
  ...
  out.innerHTML = `
    ...
    <div style="background:var(--cream);border-radius:14px;padding:20px;border:1px solid var(--border);">
      <h4 style="font-family:'Fraunces',serif;font-size:18px;margin-bottom:12px;color:var(--navy);">✈️ Quick Facts</h4>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;">
        <div style="font-size:14px;padding:10px 14px;background:var(--white);border-radius:8px;border:1px solid var(--border);">💰 <strong>${currencies}</strong></div>
        <div style="font-size:14px;padding:10px 14px;background:var(--white);border-radius:8px;border:1px solid var(--border);">🗣️ <strong>${languages}</strong></div>
        <div style="font-size:14px;padding:10px 14px;background:var(--white);border-radius:8px;border:1px solid var(--border);">🌍 <strong>${data.region}</strong></div>
        <div style="font-size:14px;padding:10px 14px;background:var(--white);border-radius:8px;border:1px solid var(--border);">🏙️ Capital: <strong>${capital}</strong></div>
      </div>
    </div>`;
}
...
function renderMap(dest) {
  const lat = window._destLat || 20;
  const lon = window._destLon || 0;
  ...
  L.marker([lat,lon],{icon})
   .addTo(window._tripMap)
   .bindPopup(`<strong style="font-family:serif;font-size:13px;">📍 ${dest}</strong><br><span style="font-size:12px;">Your destination</span>`)
   .openPopup();
}
...
function renderPhrases(phrases, dest) {
  const out = document.getElementById("phrases-out");
  out.innerHTML = `
...
function copyPhrase(text, el) {
  ...
  setTimeout(()=>{
    ...
    el.querySelector(".phrase-copy").textContent="📋 Click to copy";
  },1500);
}
...
function renderPacking(items) {
  const out = document.getElementById("packing-out");
  out.innerHTML = `
...
function renderEmergency(data, dest) {
  const out = document.getElementById("emergency-out");
  out.innerHTML = `
    <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:12px;padding:14px 18px;margin-bottom:18px;display:flex;gap:12px;align-items:flex-start;">
      ...
      <div>
        <div style="font-weight:700;color:#c0392b;font-size:14px;">Emergency Contacts — ${escHtml(dest)}</div>
        <div style="font-size:13px;color:var(--gray);margin-top:4px;">Save these before you travel. Dial the international access code first from abroad.</div>
      </div>
    </div>
    ...
    <div style="margin-top:18px;background:rgba(26,111,168,.06);border-radius:12px;padding:14px 18px;border:1px solid rgba(26,111,168,.15);">
      <h4 style="color:var(--sky);margin-bottom:8px;font-size:14px;">🛡️ Safety Tips</h4>
      <ul style="font-size:13px;color:var(--gray);padding-left:16px;line-height:2.1;">
        <li>Keep digital AND physical copies of your passport</li>
        <li>Share your itinerary with someone at home</li>
        <li>Buy travel insurance before your trip</li>
        <li>Keep emergency cash in a separate location</li>
        <li>Know your accommodation address in the local language</li>
      </ul>
    </div>`;
}
  ...
  out.innerHTML = `
    <div style="background:linear-gradient(135deg,var(--navy),var(--sky));border-radius:16px;padding:28px;color:#fff;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">
      ...
      <div style="text-align:right;">
        <div style="font-size:13px;opacity:.7;">Per Day (1 person)</div>
        <div style="font-family:'Fraunces',serif;font-size:38px;font-weight:900;">$${perDay.toFixed(0)}</div>
        <div style="font-size:12px;opacity:.6;">${data.currency||"USD"} approx.</div>
      </div>
    </div>
    <h3 style="font-family:'Fraunces',serif;font-size:20px;color:var(--navy);margin-bottom:16px;">Daily Breakdown</h3>
    ...
    ${data.note?`<div style="margin-top:18px;padding:12px 16px;background:rgba(201,150,58,.08);border-radius:10px;border:1px solid rgba(201,150,58,.2);font-size:13px;color:var(--gray);">💡 ${escHtml(data.note)}</div>`:""}`;
}

/* ══════════════════════════════════════
   RENDER: FESTIVALS
══════════════════════════════════════ */
function renderFestivals(text, dest) {
  const out  = document.getElementById("festivals-out");
  const html = text
    .replace(/##\s+(.+)/g,"<h2>$1</h2>")
    .replace(/###\s+(.+)/g,"<h3>$1</h3>")
...
function downloadItinerary() {
  const dest = window._tripDest||"Trip";
  const text = window._itineraryRaw||"No itinerary generated yet.";
  const content = [
    "═══════════════════════════════════════",
    `  TRIPARCHITECT — ${dest.toUpperCase()}`,
    "  by Gargi Patel",
    "═══════════════════════════════════════",
    "",text,"",
    "═══════════════════════════════════════",
    "  gargiptl14@gmail.com",
    "  TripArchitect © 2025",
    "═══════════════════════════════════════",
  ].join("\n");
...
function sendMsg() {
  ...
  const mailto = `mailto:gargiptl14@gmail.com?subject=${encodeURIComponent(subject||"TripArchitect Message from "+name)}&body=${encodeURIComponent(`From: ${name} (${email})\n\n${msg}`)}`;
