const http = require("http");
const fs   = require("fs");
const path = require("path");

// ── Load .env manually (no external packages needed) ──
function loadEnv() {
  try {
    const raw = fs.readFileSync(path.join(__dirname, ".env"), "utf8");
    raw.split("\n").forEach(line => {
      line = line.trim();
      if (!line || line.startsWith("#")) return;
      const [key, ...rest] = line.split("=");
      if (key && rest.length) {
        process.env[key.trim()] = rest.join("=").trim();
      }
    });
  } catch (e) {
    console.warn("⚠️  Could not load .env file:", e.message);
  }
}

loadEnv();

const PORT = process.env.PORT || 3000;

// ── Config object built from .env ──
const ENV = {
  COHERE_API_KEY:   process.env.COHERE_API_KEY   || "",
  APP_NAME:         process.env.APP_NAME          || "TripArchitect",
  AUTHOR:           process.env.AUTHOR            || "Gargi Patel",
  VERSION:          process.env.APP_VERSION        || "1.0.0",
  COHERE_ENDPOINT:  "https://api.cohere.com/v2/chat",
  WEATHER_GEO:      "https://geocoding-api.open-meteo.com/v1/search",
  WEATHER_API:      "https://api.open-meteo.com/v1/forecast",
  COUNTRY_API:      "https://restcountries.com/v3.1/name",
  TRANSLATE_API:    "https://api.mymemory.translated.net/get",
};

// ── MIME types ──
const MIME = {
  ".html": "text/html",
  ".css":  "text/css",
  ".js":   "application/javascript",
  ".json": "application/json",
  ".png":  "image/png",
  ".jpg":  "image/jpeg",
  ".svg":  "image/svg+xml",
  ".ico":  "image/x-icon",
  ".txt":  "text/plain",
};

const server = http.createServer((req, res) => {
  // ── Special route: inject ENV config as JS ──
  if (req.url === "/config.js" || req.url === "/__config") {
    const configJS = `
// Auto-generated by server.js from .env — do NOT edit manually
const ENV = ${JSON.stringify(ENV, null, 2)};
Object.freeze(ENV);
`;
    res.writeHead(200, { "Content-Type": "application/javascript" });
    res.end(configJS);
    return;
  }

  // ── Serve static files ──
  let filePath = "." + req.url;
  if (filePath === "./") filePath = "./index.html";

  const ext  = path.extname(filePath).toLowerCase();
  const mime = MIME[ext] || "application/octet-stream";

  fs.readFile(filePath, (err, data) => {
    if (err) {
      res.writeHead(404, { "Content-Type": "text/plain" });
      res.end("404 — File Not Found");
      return;
    }
    res.writeHead(200, { "Content-Type": mime });
    res.end(data);
  });
});

server.listen(PORT, () => {
  console.log("═══════════════════════════════════════════");
  console.log("  ✈️  TripArchitect is running!");
  console.log(`  🌍  Open: http://localhost:${PORT}`);
  console.log(`  🔑  API Key: ${ENV.COHERE_API_KEY ? "✅ Loaded from .env" : "❌ MISSING — add COHERE_API_KEY to .env"}`);
  console.log("  🛑  To stop: press Ctrl + C");
  console.log("═══════════════════════════════════════════");
});